...
angular.module('coreBOSAPIservice', [])
  ...
  .factory('coreBOSWSAPI', function($http, md5, coreBOSAPIStatus) {
		 ...
		 */
		corebosAPI.getVitalParameters = function(id) {
			var getdata = {
				'operation'    : 'getvitalparameters',
				'sessionName'  : coreBOSAPIStatus.getSessionInfo()._sessionid,
                                'id'  : id
			};
			return $http({
		 ...
		 * Get filter fields to show in listview
		 */
		corebosAPI.getLanguageFile = function(module) {
			var getdata = {
				'operation'    : 'getlanguagefile',
				'sessionName'  : coreBOSAPIStatus.getSessionInfo()._sessionid,
                                'module'  : module
			};
			return $http({
				method : 'GET',
				url : _serviceurl+'?lng=en&ns=translation',
				params: getdata
			});
		};
                /**
